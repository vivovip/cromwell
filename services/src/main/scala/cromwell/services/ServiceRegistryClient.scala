...
private object ServiceRegistryInstance {
  ...
  val ServiceRegistryActorInstance = mainSpecPrint("ServiceRegistryInstance init actorSystem") {
    actorSystem.actorOf(ServiceRegistryActor.props(ConfigFactory.load()), "ServiceRegistryActor")
  }
}
...
trait ServiceRegistryClient {
  ...
  import cromwell.MainSpecDebug._
  mainSpecPrint("ServiceRegistryClient init()") {
    println("FINDME: Created ServiceRegistryClient for " + this.getClass)
  }
}
