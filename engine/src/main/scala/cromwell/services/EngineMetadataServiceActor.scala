...
case class EngineMetadataServiceActor(serviceConfig: Config, globalConfig: Config) extends Actor with ActorLogging {
  ...
  private def validateWorkflowId(validation: ValidateWorkflowIdAndExecute): Unit = {
    val uuid = validation.uuid
    val requestContext = validation.requestContext
    dataAccess.workflowExists(uuid) onComplete {
      case Success(true) =>
        validation.onRecognized(WorkflowId(uuid))(requestContext)
      case Success(false) =>
        validation.onUnrecognized(uuid)(requestContext)
      case Failure(t) =>
        validation.onFailure(uuid, t)(requestContext)
    }
  }
