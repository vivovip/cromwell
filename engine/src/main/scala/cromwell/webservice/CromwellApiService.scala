...
trait CromwellApiService extends HttpService with PerRequestCreator with ServiceRegistryClient {
  ...
  private def malformedWorkflowId(id: String) = failBadRequest(new RuntimeException(s"Invalid workflow ID: '$id'."))

  private def unrecognizedWorkflowId(uuid: UUID): Route = respondWithMediaType(`application/json`) {
    failBadRequest(new RuntimeException(s"Unrecognized workflow ID: $uuid"), StatusCodes.NotFound)
  }

  private def failedWorkflowIdLookup(uuid: UUID, t: Throwable): Route = failBadRequest(new RuntimeException(s"Failed lookup attempt for ID $uuid", t))
  ...
  private def withRecognizedWorkflowId(possibleWorkflowId: String)(onRecognized: WorkflowId => Route): Route = {
    requestContext =>
      Try(UUID.fromString(possibleWorkflowId)) match {
        case Success(w) =>
          val message = ValidateWorkflowIdAndExecute(w, requestContext, onRecognized = onRecognized, onUnrecognized = unrecognizedWorkflowId, onFailure = failedWorkflowIdLookup)
          serviceRegistryActor ! message
        case Failure(ex) => malformedWorkflowId(possibleWorkflowId)(requestContext)
      }
  }
